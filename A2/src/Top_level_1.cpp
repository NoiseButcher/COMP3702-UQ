   ...
   //Best single race.
    for (k = 0; k < beta; k++) {
        if (results[k].profitOne > single.profit) {
            ...
            for (z = 0; z < single.numCycles; z++) {
                single.display.push_back(results[k].CycleName[z]);
            }
        }
    }
    ...
    for (k = 0; k < beta; k++) {
        if (results[k].profitTwo > d_ouble.profit) {
            ...
            for (z = 0; z < d_ouble.numCycles; z++) {
                if (z >= d_ouble.display.size()) {
                    d_ouble.display.push_back(results[k].CycleName[z]);
                } else {
        ...
        for (j = 0; j < beta; j++) {
            if (results[k].profitOne + results[j].profitOne > single.profit  && (k!=j)
             && (results[k].Tnames[0] != results[j].Tnames[0])) {
                ...
                useFlag = 0;
                for (z = 0; z < results[j].sizeOf; z++) {
                    for (tc = 0; tc < d_ouble.display.size(); tc++) {
                        if (d_ouble.display[tc] == results[j].CycleName[z]) {
                            useFlag++;
                            d_ouble.numCycles--;
                        }
                    }
                    if (useFlag == 0) {
                        if (z >= d_ouble.display.size()) {
                            d_ouble.display.push_back(results[j].CycleName[z]);
                        } else {
                            d_ouble.display[z] = (results[j].CycleName[z]);
                    }
                    }
                }
                useFlag = 0;
    ...
    for (k = 0; k < beta; k++) {
        if (results[k].profitThree > triple.profit) {
            ...
            for (z = 0; z < triple.numCycles; z++) {
                if (z >= triple.display.size()) {
                    triple.display.push_back(results[k].CycleName[z]);
                } else {
            ...
            if (results[k].profitOne + results[j].profitTwo > triple.profit && (k!=j)
                ...
                for (z = 0; z < results[j].sizeOf; z++) {
                    for (tc = 0; tc < triple.display.size(); tc++) {
                        if (triple.display[tc] == results[j].CycleName[z]) {
                            useFlag++;
                            triple.numCycles--;
                        }
                    }
                    if (useFlag == 0) {
                        if (z >= triple.display.size()) {
                            triple.display.push_back(results[j].CycleName[z]);
                        } else {
                            triple.display[z] = (results[j].CycleName[z]);
                        }
                    }
                }
                useFlag = 0;
            ...
            for (i = 0; i < beta; i++) {
                if (results[k].profitOne + results[j].profitOne + results[i].profitOne > triple.profit
                    ...
                    && (results[j].Tnames[0] != results[i].Tnames[0])) {
                            ...
                            useFlag = 0;
                            for (z = 0; z < results[i].sizeOf; z++) {
                                for (tc = 0; tc < triple.display.size(); tc++) {
                                    if (triple.display[tc] == results[i].CycleName[z]) {
                                        useFlag++;
                                        triple.numCycles--;
                                    }
                                }
                                if (useFlag == 0) {
                                    if (z >= triple.display.size()) {
                                        triple.display.push_back(results[i].CycleName[z]);
                                    } else {
                                        triple.display[z] = (results[i].CycleName[z]);
                                    }
                                }
                            }
                            useFlag = 0;
    ...
    if (single.profit >= d_ouble.profit && single.profit >= triple.profit) {
        ...
        outFile << tracks[d_ouble.indexes[0]].name[0] << " " << tracks[d_ouble.indexes[0]].numCycles << " ";
